...
public enum ClaimSourceKind
{
    ...
    /// <summary>The folder the file sits in, or what the rest of that folder turned out to be.</summary>
    Folder
}
...
/// </param>
public sealed record ClaimSource(ClaimSourceKind Kind, string Detail, bool IsDeliberate = false)
{
    ...
    /// <remarks>
    /// It is read off sibling files rather than this one, so it is not independent of them: a folder
    /// agreeing with the file names it was computed from is one source, counted twice.
    /// </remarks>
    public static ClaimSource FolderAgreement { get; } = new(ClaimSourceKind.Folder, "the rest of the folder");
}
