...
| `QueueConsumptionService` | Dequeues items, drives playback, tracks elapsed time, records history, and holds the gap between two dances. The gap is a wait rather than a queue item: nothing is dequeued while it runs, so the coming dance is still what the screens call next, and nothing has to filter it out of the queue, the remote, the history or the row indices a move works on. `TrackGaps` is the same rule for anything projecting when the evening ends. |
